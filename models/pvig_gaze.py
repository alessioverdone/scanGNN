...
def pvig_ti_224_gelu(pretrained=False, **kwargs):
    ...
    model.prediction[-1] = nn.Conv2d(model.prediction[-1].in_channels, 2, kernel_size=(1, 1),
                                     stride=(1, 1))  # ouc_channel was 3
...
def pvig_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            ...
            self.norm = 'batch'  # batch or instance normalization {batch, instance}
            self.bias = True  # bias of conv layer True or False
            self.dropout = 0.1  # dropout rate, 0.0
            self.use_dilation = True  # use dilated knn or not
            self.epsilon = 0.2  # stochastic epsilon for gcn
            self.use_stochastic = False  # stochastic for gcn, True or False
            self.drop_path = drop_path_rate
    ...
    model.prediction[-1] = nn.Conv2d(model.prediction[-1].in_channels, 2, kernel_size=(1, 1), stride=(1, 1))
    m = model.prediction[-1]
    torch.nn.init.kaiming_normal_(m.weight)
    m.weight.requires_grad = True
    if m.bias is not None:
...
def pvig_s_224_gelu_2(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            self.k = 4  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            self.act = 'gelu'  # activation layer {relu, prelu, leakyrelu, gelu, hswish}
            self.norm = 'batch'  # batch or instance normalization {batch, instance}
            self.bias = True  # bias of conv layer True or False
            self.dropout = 0.4  # dropout rate, 0.0
            self.use_dilation = True  # use dilated knn or not
            self.epsilon = 0.2  # stochastic epsilon for gcn
            self.use_stochastic = False  # stochastic for gcn, True or False
            self.drop_path = drop_path_rate
            self.imagesize = 224
            self.pyramid_levels = [2, 4, 6]
            self.blocks = [1, 1, 3, 1]  # number of basic blocks in the backbone
            self.channels = [40, 80, 200, 320]  # number of channels of deep features
            self.n_classes = num_classes  # Dimension of out_channels
            self.emb_dims = 256  # Dimension of embeddings

    opt = OptInit(**kwargs)
    opt.dropout = kwargs.get("dropout", 0.1)
    opt.blocks = kwargs.get("blocks", [1, 1, 3, 1])
    opt.pyramid_levels = kwargs.get("pyramid_levels", [2, 4, 6])
    opt.emb_dims = kwargs.get("emb_dims", 256)
    print('opt.dropout: ', str(opt.dropout))
    print('opt.blocks: ', str(opt.blocks))
    print('opt.pyramid_levels: ', str(opt.pyramid_levels))
    print('opt.emb_dims: ', str(opt.emb_dims))
